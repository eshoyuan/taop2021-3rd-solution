...
def quadratic_weighted_kappa(rater_a, rater_b, min_rating=None, max_rating=None):
    ...
    """
    E = [[0 for i in range(num_ratings)]
                for j in range(num_ratings)]
    for i in range(num_ratings):
        for j in range(num_ratings):
            E[i][j] = hist_rater_a[i]*hist_rater_b[j]/num_scored_items
...
def cls_accuracy(rater_a, rater_b):
    rater_a = np.array(rater_a, dtype=int)
    rater_b = np.array(rater_b, dtype=int)
    x = float(np.sum(rater_a==rater_b))
    y = float(len(rater_a))
    return x/y
...
if __name__ == "__main__":
    ...
    rater_a = f.read()
    """
    rater_a = np.load("predictions.npy")
    rater_b = np.load("labels.npy")
    m1 = quadratic_weighted_kappa(rater_a, rater_b)
    m2 = cls_accuracy(rater_a, rater_b)
    print('Auadratic_weighted_kappais', m1)
    print('Accuracy is', m2)